...
function mode(obj){
		var $nav=obj.find(".main .head .nav ul li");
		var $list=obj.find(".main .content .right .list");
		var $li=$list.find("li");
		...
		$title.each(function(){
			if($(this).text().length>15){
				var str=$(this).text().substring(0,15)+'...';
				$(this).html(str);
			}
	    });
	    $tip.each(function(i){
	...
	function mode1(obj){
		var $content=obj.find(".main .content");
        var $pic=$content.find(".pic");
		var $span=obj.find(".main .head .btn span");
		var timer=null;
	    var index=0;
	    auto();
		function auto(){
